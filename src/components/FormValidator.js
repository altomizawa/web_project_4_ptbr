export class FormValidator {
  constructor(config, formToValidate) {
    ...
    this.submitButton = this.formToValidate.querySelector(
      this.config.submitButtonSelector
    );
    this.inputs = Array.from(
      this.formToValidate.querySelectorAll(this.config.inputSelector)
    );
  ...
  _showErrorClass(input) {
    const errorElement = this.formToValidate.querySelector(
      `#${input.id}--error`
    );
